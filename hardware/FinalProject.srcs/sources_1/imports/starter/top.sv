...
`endif
   ...
   // new timer module
   wire [wordsize-1:0] millisecond_time;
   timer #(.wordsize(wordsize)) my_timer(.clk12(clk12), .millisecond_time(millisecond_time));
   ...
   memIO #(.wordsize(wordsize), .dmem_size(dmem_size), .dmem_init(dmem_init), .Nchars(Nchars), .smem_size(smem_size), .smem_init(smem_init)
       ...
       .vga_addr(smem_addr), .vga_readdata(charcode), .keyb_char, .accel_val, .period, .lights(LED), .millisecond_time(millisecond_time));
